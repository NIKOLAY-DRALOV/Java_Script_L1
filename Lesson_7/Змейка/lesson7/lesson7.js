...
function haveFood(unit) {
    ...
    // Если корм подобран то возникает препятствие
    if (unit_classes.includes('food-unit')) {
        check = true;
        createFood();
        score++;
        createLet()
        setInterval(gameScore);
    } else if (unit_classes.includes('let-unit')){
        check = true;
        finishTheGame()
    }
    return check;
...
// Создание препятсятвия
function createLet() {
    var letCreated = false;

    while (!letCreated) { //пока препятствие не создали
        // рандом
...
// Функция подсчета очков в режиме реального времени
    function gameScore(){
    document.querySelector(".game_score").innerHTML = "Ваш результат: " + score;
}
